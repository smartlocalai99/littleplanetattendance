...
export function averageFaceDescriptors(descriptors) {
  const normalized = descriptors
    .map((descriptor) => normalizeStoredFaceDescriptor(descriptor))
    .filter(Boolean);
  ...
  if (normalized.some((descriptor) => descriptor.vector.length !== vectorLength)) {
    return null;
  }
  ...
  for (const descriptor of normalized) {
    descriptor.vector.forEach((value, index) => {
      vector[index] += value;
    });
  }
  ...
  return {
    ...
    vector: vector.map((value) =>
      Number((value / normalized.length).toFixed(6)),
    ),
  };
}
